...
# Get current WiFi SSID for reporting
...
# Enhanced payload with additional system information including WiFi SSID
        ...
        case "$COMMAND" in
          "fetch_splash")
            ...
            if [ -f "$INSTALL_DIR/scripts/fetch-splash.sh" ]; then
              $INSTALL_DIR/scripts/fetch-splash.sh
            else
              log "Error: fetch-splash.sh not found"
            fi
            ;;
          "reboot")
